...
- **Feet mode (the default), like the Jobber.** Type the feet digits, then one key for inches (0–11), then one key for sixteenths (0–15). The keypad has single keys for 10–15 so that each of these is one press. `2 4 5 10` shows 24′ 5-10/16″ while you type and simplifies to 24′ 5-5/8″ when you press `=`. After a length, `×` and `÷` take a plain number: `1 0 0 0 ÷ 3` = 3′ 4″.
- **FT/DEC** switches to plain decimal numbers. In either mode you can put a unit key after the digits instead: `12 Feet`, `24 Feet 11 Inch 5` (after `Inch`, the next number key is sixteenths), `3 ⁄ 8 Inch`, `2 m`.
