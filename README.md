...
- **Feet mode (the default), like the Jobber.** Type the feet digits, then one key for inches (0–11), then one key for sixteenths (0–15). The keypad has single keys for 10–15 so that each of these is one press. `1 0 0 0` shows 10′-0 0/16. `2 4 5 10` shows 24′-5 10/16 while you type and simplifies to 24′-5 5/8″ when you press `=`. After a length, `×` and `÷` take a plain number: `1 0 0 0 ÷ 3` = 3′-4″.
